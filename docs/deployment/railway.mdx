---
title: "Deploying 4G3N7 on Railway"
description: "Comprehensive guide to deploying the full 4G3N7 stack on Railway using the official 1-click template"
---

> **TL;DR –** Click the button below, add your AI API key (Anthropic, OpenAI, or Google), and your personal 4G3N7 instance will be live in ~2 minutes.

[![Deploy on Railway](https://railway.com/button.svg)](https://railway.com/new/template?template=4g3n7&referralCode=L9lKXQ)

---

## Why Railway?

Railway provides a zero-ops PaaS experience with private networking and per-service logs that perfectly fits 4G3N7’s multi-container architecture. The official template wires every service together using the latest container images pushed to the `edge` branch.
...
| ---------------- | -------------------------------------------------------------------- | ---- | -------- | ------------------------------------ |
| **4g3n7-ui**   | `ghcr.io/4g3n7-ai/4g3n7-ui:edge`                                 | 9992 | **Yes**  | Next.js web UI rendered to the world |
| **4g3n7-agent**| `ghcr.io/4g3n7-ai/4g3n7-agent:edge`                              | 9991 | No       | Task orchestration & LLM calls       |
| **4g3n7-desktop**| `ghcr.io/4g3n7-ai/4g3n7-desktop:edge`                          | 9990 | No       | Containerised Ubuntu + XFCE desktop  |
| **postgres**     | `postgres:14-alpine`                                                 | 5432 | No       | Persistence layer                    |

All internal traffic flows through Railway’s [private networking](https://docs.railway.com/guides/private-networking). Only `4g3n7-ui` is assigned a public domain.
...
<Steps>
  <Step title="1. Open the Template">
    Click the **Deploy on Railway** button above or visit [https://railway.com/new/template?template=4g3n7&referralCode=L9lKXQ](https://railway.com/new/template?template=4g3n7&referralCode=L9lKXQ).
  </Step>
  <Step title="2. Configure Environment">
    For the 4g3n7-agent resource, add your AI API key (choose at least one):
  ...
  <Step title="4. Launch 4G3N7">
    When the build logs show *"4g3n7-ui: ready"*, click the generated URL (e.g. `https://4g3n7-ui-prod.up.railway.app`). You should see the task interface. Create a task and watch the desktop stream!  
...
| ------- | ------------ | ---- |
| Web UI shows “connecting…” | Desktop not ready or private networking mis-config | Wait for `4g3n7-desktop` container to finish starting, or restart service |
