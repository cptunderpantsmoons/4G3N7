...
description: "Use any LLM provider with 4G3N7 through LiteLLM proxy"
---

# Connect Any LLM to 4G3N7 with LiteLLM

LiteLLM acts as a unified proxy that lets you use 100+ LLM providers with 4G3N7 - including Azure OpenAI, AWS Bedrock, Anthropic, Hugging Face, Ollama, and more. This guide shows you how to set up LiteLLM with 4G3N7.
...
## Quick Start with 4G3N7's Built-in LiteLLM Proxy

4G3N7 includes a pre-configured LiteLLM proxy service that makes it easy to use any LLM provider. Here's how to set it up:
...
<Steps>
  <Step title="Use Docker Compose with Proxy">
    ...
    # Start 4G3N7 with LiteLLM proxy
    docker-compose -f docker/docker-compose.proxy.yml up -d
    ```
    
    This automatically:
    - Starts the `4g3n7-llm-proxy` service on port 4000
    - Configures the agent to use the proxy via `G3N7_LLM_PROXY_URL`
  ...
  <Step title="Customize Model Configuration">
    ...
    ```yaml
    # packages/4g3n7-llm-proxy/litellm-config.yaml
    model_list:
      # Add Azure OpenAI
  ...
  <Step title="Verify Models are Available">
    The 4G3N7 agent automatically queries the proxy for available models:
    
    ```bash
    # Check available models through 4G3N7 API
...
```mermaid
graph LR
    A[4G3N7 UI] -->|Select Model| B[4G3N7 Agent]
    B -->|G3N7_LLM_PROXY_URL| C[LiteLLM Proxy :4000]
...
1. **4g3n7-llm-proxy Service**: A LiteLLM instance running in Docker that:
   - Runs on port 4000 within the 4G3N7 network
   - Uses the config from `packages/4g3n7-llm-proxy/litellm-config.yaml`
   - Inherits API keys from environment variables

2. **Agent Integration**: The 4G3N7 agent:
   - Checks for `G3N7_LLM_PROXY_URL` environment variable
...
# Use "smart-model" in 4G3N7 config
```
...
services:
  4g3n7-agent:
    environment:
      ...
      - G3N7_LLM_PROXY_URL=http://your-litellm-server:4000
    # ... rest of config
```
...
export G3N7_LLM_PROXY_URL=http://your-litellm-server:4000
...
# Then start 4G3N7 with:
export G3N7_LLM_PROXY_URL=http://localhost:4000
docker-compose up -d
```
...
<AccordionGroup>
  ...
  <Accordion title="Authentication errors">
    Verify master key in both LiteLLM and 4G3N7:
    ```bash
    # Test LiteLLM
...
<Note>
  **Pro tip:** Start with a single provider, then add more as needed. LiteLLM makes it easy to switch or combine models without changing 4G3N7 configuration.
</Note>