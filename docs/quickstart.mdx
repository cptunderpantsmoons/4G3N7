...
4G3N7 can be deployed in several ways depending on your needs:
...
<Steps>
  <Step title="Visit the Template">
    Click the Deploy Now button in the 4G3N7 template on Railway.
  </Step>
  <Step title="Add Anthropic Key">
    Enter either your `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, or `GEMINI_API_KEY` for the 4g3n7-agent resource.
  </Step>
...
- **4G3N7 Desktop**: Containerized Linux environment
...
Navigate to [http://localhost:9992](http://localhost:9992) to access the 4G3N7 UI.

**Two ways to interact:**
1. **Tasks**: Enter task descriptions to have 4G3N7 work autonomously
...
<Note>
  **🔐 Password Manager Support**: 4G3N7 can handle authentication automatically when you install a password manager extension. See our [password management guide](/guides/password-management) for setup instructions.
</Note>
...
Now let's see 4G3N7 in action! Try these example tasks:
...
# Just the agent
docker-compose -f docker/docker-compose.yml logs -f 4g3n7-agent
```
...
curl -X POST http://localhost:9990/computer-use \
  -H "Content-Type: application/json" \
  -d '{"action": "type_text", "text": "Hello, 4G3N7!"}'
```
...
<AccordionGroup>
  ...
  <Accordion title="Agent errors or no response">
    ...
    ```bash 
    cat docker/.env
    docker-compose -f docker/docker-compose.yml logs 4g3n7-agent 
    ```
...
<CardGroup cols={2}>
  ...
  <Card title="Takeover Mode" icon="hand" href="/guides/takeover-mode">
    Take control when you need to guide 4G3N7
  </Card>
  ...
  <Card title="API Integration" icon="code" href="/api-reference/introduction">
    Automate 4G3N7 with your applications
  </Card>
</CardGroup>
