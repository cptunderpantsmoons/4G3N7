...
// Backend URLs
const G3N7_AGENT_BASE_URL = process.env.G3N7_AGENT_BASE_URL;
const G3N7_DESKTOP_VNC_URL = process.env.G3N7_DESKTOP_VNC_URL;
...
app
  ...
  .then(() => {
    ...
    expressApp.use("/api/proxy/websockify", (req, res) => {
      ...
      // Rewrite path
      const targetUrl = new URL(G3N7_DESKTOP_VNC_URL!);
      req.url =
        targetUrl.pathname +
