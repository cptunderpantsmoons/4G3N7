...
export class ComputerUseService {
  ...
  private async writeFile(
    ...
    try {
      ...
      const tempFile = `/tmp/4g3n7_temp_${Date.now()}_${Math.random().toString(36).substring(7)}`;
  ...
  private async readFile(action: ReadFileAction): Promise<{
    ...
    try {
      ...
      const tempFile = `/tmp/4g3n7_read_${Date.now()}_${Math.random().toString(36).substring(7)}`;
