...
import { DEFAULT_MODEL } from './qwen.constants';
...
export class QwenService implements BytebotAgentService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    const baseURL =
      this.configService.get<string>('QWEN_API_BASE') ||
  ...
  async generateMessage(
    ...
    try {
      ...
      return {
        contentBlocks: this.formatResponse(response.output),
        tokenUsage: {
      ...
      if (error instanceof APIUserAbortError) {
        this.logger.log('Qwen API call aborted');
        throw new BytebotAgentInterrupt();
      }
      this.logger.error(
        `Error sending message to Qwen: ${error.message}`,
        error.stack,
      );
      throw error;
    }
